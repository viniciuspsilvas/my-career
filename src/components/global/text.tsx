...
export interface TextProps
  extends HTMLAttributes<unknown>,
  ...
  switch (category) {
    ...
    case "h1":
      return (
        <h1
          className={cn(
            "text-4xl sm:text-5xl font-bold text-gray-900 dark:text-white relative z-10 font-poppins",
    ...
    case "h2":
      return (
        <h2
          {...rest}
          className={cn(
            "text-2xl font-bold leading-tight text-gray-900 dark:text-white",
            variants({ status, className }),
          )}
        >
          {children}
    ...
    case "label":
      return (
        <label
          className={cn("label", variants({ status, className }))}
          {...rest}
        >
          <span className="label-text text-sm text-gray-500 dark:text-gray-400">
            {children}
          </span>
        </label>
      );
    ...
    default:
      return (
        <span className={cn(variants({ status, className }))}>{children}</span>
      );
  }
};
