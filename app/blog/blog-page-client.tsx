          ...
          <Link key={index} href={`/blog/${post._id}`} passHref>
           ...
           >
             ...
             <div className="flex flex-col justify-between h-full gap-4">
                <Text category="h2" className="mb-4 line-clamp-2 sm:line-clamp-3 text-xl font-bold text-gray-900 dark:text-gray-100">{post.title}</Text>
                <Text category="p1" className="line-clamp-3 sm:line-clamp-5 text-gray-700 dark:text-gray-300">{post.content}</Text>
                <Text category="small" status='basic' className="line-clamp-3 sm:line-clamp-5 text-gray-500 dark:text-gray-400">{post?.publishedAt && getHumanReadableDateFormat(post.publishedAt)}</Text>
             </div>
           </motion.div>
         </Link>
        ))}
